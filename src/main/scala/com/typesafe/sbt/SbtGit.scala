...
//    gitUncommittedChanges in ThisBuild := gitReader.value.withGit(_.hasUncommittedChanges)
  ...
  val projectSettings = Seq(
    ...
    gitRunner := ConsoleGitRunner,
    gitHeadCommit <<= (gitReader in ThisProject)( gr => gr.withGit(_.headCommitSha)),
    gitTagToVersionNumber := git.defaultTagByVersionStrategy,
    gitDescribedVersion <<= (gitReader in ThisProject,git.gitTagToVersionNumber) ( (gr,g2v) => gr.withGit(_.describedVersion).map{v => g2v(v).getOrElse(v)}),
    gitCurrentTags := gitReader.value.withGit(_.currentTags),
    gitCurrentBranch := Option(gitReader.value.withGit(_.branch)).getOrElse(""),
    gitUncommittedChanges in ThisBuild := gitReader.value.withGit(_.hasUncommittedChanges)
  )
  ...
  def versionWithGit: Seq[Setting[_]] =
    Seq(
        ...
        formattedDateVersion in ThisProject := {
          val base = git.baseVersion.?.value
          git.defaultFormatDateVersion(base, new java.util.Date)
        },
        isSnapshot in ThisProject <<= (git.gitCurrentTags in ThisProject,git.gitUncommittedChanges in ThisProject) { (gct,guc) =>
          gct.isEmpty || guc
        },
        version in ThisProject := {
          val overrideVersion =
  ...
  object git {
    ...
    val runner = GitKeys.gitRunner in ThisProject
    val gitHeadCommit = GitKeys.gitHeadCommit in ThisProject
    val useGitDescribe = GitKeys.useGitDescribe in ThisProject
    val gitDescribedVersion = GitKeys.gitDescribedVersion in ThisProject
    val gitCurrentTags = GitKeys.gitCurrentTags in ThisProject
    val gitCurrentBranch = GitKeys.gitCurrentBranch in ThisProject
    val gitTagToVersionNumber = GitKeys.gitTagToVersionNumber in ThisProject
    val baseVersion = GitKeys.baseVersion in ThisProject
    val versionProperty = GitKeys.versionProperty in ThisProject
    val gitUncommittedChanges = GitKeys.gitUncommittedChanges in ThisProject
    val uncommittedSignifier = GitKeys.uncommittedSignifier in ThisProject
    val formattedShaVersion = GitKeys.formattedShaVersion in ThisProject
    val formattedDateVersion = GitKeys.formattedDateVersion in ThisProject
...
//  override def buildSettings = GitPlugin.autoImport.versionWithGit
  override def projectSettings: Seq[Def.Setting[_]] = GitPlugin.autoImport.versionWithGit
}
